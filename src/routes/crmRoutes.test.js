...
test('Super Admin CRM routes include operational screens and exclude legacy campaign screens', () => {
  ...
  expect(paths).toEqual(expect.arrayContaining(['/contacts', '/contacts/new', '/contacts/import', '/leads', '/leads/unassigned', '/reassignment-requests', '/staff-users', '/whatsapp-templates', '/whatsapp-templates/:templateId', '/whatsapp-inbox', '/whatsapp-broadcasts']));
...
test('Counsellor routes expose assigned workflows but not global administration', () => {
  ...
  expect(paths).toEqual(expect.arrayContaining(['/dashboard', '/my-leads', '/contacts/new', '/contacts/:contactId', '/reassignment-requests', '/whatsapp-templates', '/whatsapp-templates/:templateId', '/whatsapp-inbox']));
