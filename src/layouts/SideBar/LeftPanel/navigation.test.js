...
test('Super Admin navigation includes every Phase 1B administrator workflow', () => {
  expect(navigationForRole('SUPER_ADMIN').map((item) => item.name)).toEqual([
    ...
    'Unassigned Leads', 'Needs Contact', 'Interested Leads', 'Reassignment Requests', 'Follow-ups', 'WhatsApp Templates', 'WhatsApp Inbox', 'WhatsApp Broadcasts', 'Staff Users',
  ]);
});
...
test('counsellor navigation exposes only assigned CRM workflows', () => {
  ...
  expect(counsellorItems).toEqual(['Dashboard', 'Add Contact', 'My Leads', 'Reassignment Requests', 'Follow-ups', 'WhatsApp Templates', 'WhatsApp Inbox']);
});
